...
In this design, I want to mention a number of _deliberate_ design choices.

**First is where rate limiting is handled.** By placing the rate limiting logic in the API gateway, we avoid any latency from external API calls. An alternative solution might be to have a dedicated rate limit service. However, this would add latency making round-trip requests to determine whether a request should succeed. 
Another benefit of the rate limiter placement is that the API Gateway is the entry point to our application. This means all requests necessarily funnel through it. As a result, the API Gateway is aware of all requests from all users. This means we can be confident our request counts by user are accurate.
If, instead, rate limiting was handled by each service individually, there would be no global request count per user. Independent services won't know about requests made to other services, so enforcing a specific rate limit becomes unfeasible.

**Second is the choice of a distributed cache like Redis.** By using a distributed cache like Redis, we can look up the current request usage for a given user ID with an `HMGET` call which we can expect to take < 1 ms. To We can further mitigate latency by [pooling connections](https://en.wikipedia.org/wiki/Connection_pool).
Connection pooling allows for re-using existing connections so we can avoid repeated TCP handshakes, authentiation, and socket allocation and make calls to Redis even faster.

**Third is how rules are designed.** In this system, rules can be updated on the fly with no disruption using a pub/sub model. This works by adding a rule configuration service which publishes rules which are then pushed to the API Gateway. The API Gateway is subscribed to a rules channel and can manage its own rule cache for quick access. As rules are created, updated, or deleted, the API Gateway
can monitor these changes in real time. Another solution is periodic polling with a rules database. If the API Gateway polls a database every `N` hours to refresh it's rule set, rules will be out of sync for up to `N` hours. We can make `N` really small and poll every few seconds, but that adds strain on the API Gateway which is already handling 500K req/sec. Rules probably don't change very often, so many of the requests would be wasteful.
...
For low latency, there are a couple of things to note. First is our choice of using Redis. First of all, our system should not have to make any expensive requests to a database or any other API. We can encode the user ID in the request's JWT which allows us to access it without any additonal information., pool Redis connections, and lightning fast rate limiting algorithms. This should make the end-to-end check very fast.
